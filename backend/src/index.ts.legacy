...
io.on("connection", async (socket) => {
  ...
  if (userId) {
    ...
    // Join company room and internal user room for multi-tenancy isolation
    try {
      ...
      if (user) {
        if (user.id) {
          ...
          logger.info(`[Socket] 👤 User joined internal ID room: user:${user.id}`, { userId });
        }
        ...
        logger.warn("[Socket] ⚠️  User not found in public.users table", { socketId: socket.id, userId });
      }
    } catch (error) {
