          ...
          if (!contactContext) {
            try {
              ...
              contactContext = {
                ...
                tags: tagsRes.data?.map((t: { tag_id: string }) => t.tag_id) || []
              };
            } catch (_err) {
              logger.error("[FlowEngine] Error fetching contact context for filters", _err);
...
async function startFlowExecution(
  ...
  // Find the start node
  const startNode = (flow.nodes as unknown as FlowNode[]).find((n: FlowNode) => n.type === 'trigger' || (n.data as Record<string, unknown>)?.isStart);
  if (!startNode) {
  ...
  // Create execution
  const execution: FlowExecution = await flowsRepo.createExecution({
    flow_id: flow.id,
    ...
    variables: { 
      ...
      inbox_id: (triggerData?.inbox_id as string) || (flow.trigger_config as Record<string, unknown>)?.inbox_id as string
    }
  });
 ...
 */
export async function queueNextStep(executionId: string): Promise<void> {
 ...
 */
export async function resumeFlowWithResponse(chatId: string, message: Record<string, unknown>): Promise<boolean> {
  ...
  const executionList = (executions || []) as unknown as { id: string, variables: Record<string, unknown> }[];
...
export async function processFlowStep(executionId: string): Promise<void> {
  ...
  const currentNode = (flow.nodes as unknown as FlowNode[]).find((n: FlowNode) => n.id === currentNodeId);
  ...
  try {
    ...
    // Find next node
    const edge = (flow.edges as unknown as FlowEdge[]).find((e: FlowEdge) => e.source === currentNodeId && (!result.edgeHandle || e.sourceHandle === result.edgeHandle));
    ...
    await flowsRepo.updateExecution(executionId, { 
      ...
      variables: { ...((execution.data.variables as Record<string, unknown>) || {}), ...(result.variables || {}) }
    });
    ...
    logger.error(`[FlowEngine] Error executing node ${currentNodeId}: ${errorMessage}`);
    await flowsRepo.updateExecution(executionId, { 
      status: 'FAILED', 
...
async function sendFlowMessage(args: {
  ...
  if (data.buttons?.length > 0 && inboxId && customerPhone && isMeta) {
    try {
      ...
      const { message } = await sendInteractiveButtons({
        ...
        customerPhone,
        message: textContent,
        buttons: (data.buttons as Record<string, unknown>[]).map((b, i: number) => ({
          id: b.id as string || `btn_${i}`,
  ...
  if (data.media_url) {
    ...
    if (data.buttons && (data.buttons as unknown[]).length > 0) {
      finalContent += "\n\n" + (data.buttons as Record<string, unknown>[]).map((b, i: number) => `${i + 1}️⃣ ${b.text}`).join("\n");
    } else if (data.list_sections) {
      const options = (data.list_sections as Array<{ rows: Array<{ title: string, description?: string }> }>).flatMap((s) => 
  ...
  if (textContent && buttons?.length > 0) {
    if (isMeta) {
      ...
      try {
        await sendInteractiveButtons({
          ...
          buttons: buttons.map((b, i: number) => ({
            id: (b as Record<string, unknown>).id as string || `btn_${i}`,
            title: (b as Record<string, unknown>).text as string
          })),
          senderSupabaseId: null
        });
      } catch (_err) {
        logger.error("[FlowEngine] Error sending buttons after text", _err);
      }
    } else {
      // Non-Meta: Combine both in one message
      const finalContent = textContent + "\n\n" + buttons.map((b, i: number) => `${i + 1}️⃣ ${(b as Record<string, unknown>).text}`).join("\n");
      await publish(EX_APP, "outbound.request", {
        jobType: "message.send",
        customerId: customer.id,
        chatId,
    ...
    if (isMeta) {
      try {
        ...
        // Fallback to text
        const finalContent = buttons.map((b, i: number) => `${i + 1}️⃣ ${(b as Record<string, unknown>).text}`).join("\n");
        await publish(EX_APP, "outbound.request", {
          jobType: "message.send",
          customerId: customer.id,
          chatId,
...
async function executeNode(node: FlowNode, execution: Record<string, unknown>): Promise<{ status: string, next_step_at?: string, variables?: Record<string, unknown>, edgeHandle?: string }> {
  ...
  switch (type) {
    ...
    case 'ai_action': {
      ...
      const { action, agent_id, destination_status, change_chat_status } = data as Record<string, unknown>;

      if (action === 'ACTIVATE') {
        const updateData: Record<string, unknown> = { ai_agent_id: agent_id };
        ...
        const updateData: Record<string, unknown> = { ai_agent_id: null };
        if (change_chat_status) updateData.status = change_chat_status;
 ...
 */
async function resolveRichSystemContext(companyId: string, triggerData: Record<string, unknown>): Promise<Record<string, unknown>> {
  const result: Record<string, unknown> = {};
  const entityType = triggerData.entity_type as string;
  const entityId = triggerData.entity_id as string;
