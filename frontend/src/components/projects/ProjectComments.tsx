    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="relative">
        <textarea
          ...
          className="w-full bg-(--color-bg) border border-(--color-border) rounded-lg p-4 pr-12 text-sm text-(--color-text) focus:ring-2 focus:ring-(--color-primary) outline-none resize-none min-h-[100px]"
        />
        <button
          type="submit"
          disabled={!newComment.trim() || submitting}
          className="absolute bottom-4 right-4 p-2 bg-(--color-primary) text-white rounded-lg hover:opacity-90 disabled:opacity-50 transition-colors"
        >
          <Send size={18} />
        </button>
      </form>
      ...
      <div className="space-y-4">
        {comments.length === 0 ? (
          ...
          comments.map((comment) => (
            <div
              key={comment.id}
              className="bg-(--color-surface-muted) rounded-xl p-4 border border-transparent hover:border-(--color-border) transition-all group"
            >
              <div className="flex items-start justify-between mb-2">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-(--color-primary)/10 flex items-center justify-center text-(--color-primary) font-bold text-xs">
                ...
                <button
                  ...
                  className="p-2 text-(--color-text-muted) hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all"
                >
                  <Trash2 size={16} />
                </button>
              </div>
