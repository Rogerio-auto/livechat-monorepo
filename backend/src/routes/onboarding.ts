...
export function registerOnboardingRoutes(app: Application) {
  // ROTA ÚNICA: Criar usuário + empresa de uma vez com todos os dados
  app.post("/api/onboarding/signup-complete", async (req: any, res) => {
    try {
      ...
      console.log("[signup-complete] Dados recebidos:", { 
        name, email, phone, company_name, cnpj, city, state, team_size 
      });
  ...
  app.post("/api/onboarding/create-company", requireAuth, async (req: any, res) => {
    try {
  ...
  app.post("/api/onboarding/signup", async (req: any, res) => {
    try {
  ...
  app.post("/api/onboarding/step/1", requireAuth, async (req: any, res) => {
    try {
      const companyId = await resolveCompanyId(req);
  ...
  app.post("/api/onboarding/step/2", requireAuth, async (req: any, res) => {
    try {
      const companyId = await resolveCompanyId(req);
  ...
  app.post("/api/onboarding/step/3", requireAuth, async (req: any, res) => {
    try {
      const companyId = await resolveCompanyId(req);
  ...
  app.get("/api/onboarding/industry-config/:industry", requireAuth, async (req: any, res) => {
    try {
  ...
  app.post("/api/onboarding/save-plan", requireAuth, async (req: any, res) => {
    try {
      const companyId = await resolveCompanyId(req);
