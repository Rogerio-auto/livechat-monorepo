...
import type { Chat, Message } from "@livechat/shared";
...
function EditableField({ 
  ...
  return (
    <div className="flex items-center gap-3 text-sm p-3 rounded-xl bg-(--color-surface-muted) border border-(--color-border) hover:border-(--color-primary)/30 transition-colors group relative">
      <div className="w-8 h-8 rounded-lg bg-(--color-surface) flex items-center justify-center text-(--color-text-muted) group-hover:text-(--color-primary) transition-colors shrink-0">
        <Icon size={14} />
      </div>
      
      <div className="flex-1 min-w-0">
        <div className="text-[10px] text-(--color-text-muted) mb-0.5">{label}</div>
        {isEditing ? (
          <input
            className="w-full bg-(--color-surface) border border-(--color-border) rounded px-2 py-1 text-xs text-(--color-text) focus:outline-none focus:border-(--color-primary)"
            value={draft}
          ...
          <div className={`truncate font-medium ${!value ? "text-(--color-text-muted) italic opacity-70" : "text-(--color-text)"}`}>
            {value || placeholder}
          </div>
        )}
      </div>
        ...
        {isEditing ? (
          <>
            <button 
              onClick={handleSave}
              disabled={saving}
              className="p-1.5 rounded-lg hover:bg-(--color-primary)/10 text-(--color-primary) transition-colors"
              title="Salvar"
            >
              <FiCheck size={14} />
            </button>
          ...
          <button 
            ...
            className="p-1.5 rounded-lg opacity-0 group-hover:opacity-100 hover:bg-(--color-surface) text-(--color-text-muted) hover:text-(--color-primary) transition-all"
            title="Editar"
          >
            <FiEdit2 size={14} />
          </button>
...
function MediaGallery({ chat, apiBase, type }: { chat: Chat; apiBase?: string; type: "IMAGE" | "DOCUMENT" | "LINK" }) {
  ...
  if (loading) return <div className="p-4 text-center text-xs text-(--color-text-muted)">Carregando...</div>;

  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-8 text-(--color-text-muted) opacity-60">
  ...
  if (type === "IMAGE") {
    return (
      <div className="grid grid-cols-3 gap-2 p-2">
        {items.map(m => (
          <a key={m.id} href={m.media_url || "#"} target="_blank" rel="noopener noreferrer" className="aspect-square bg-(--color-surface-muted) rounded-lg overflow-hidden border border-(--color-border) hover:opacity-80 transition-opacity">
            {m.type === "VIDEO" ? (
        ...
        <div className="flex flex-col gap-2 p-2">
           {items.map(m => {
              ...
              return links.map((link, i) => (
                 <a key={`${m.id}-${i}`} href={link} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 p-2 rounded-lg bg-(--color-surface-muted) border border-(--color-border) hover:bg-(--color-surface) transition-colors">
                    <div className="w-8 h-8 rounded bg-(--color-primary)/10 flex items-center justify-center text-(--color-primary) shrink-0">
                       <FiLink size={14} />
                    </div>
                    <div className="flex-1 min-w-0">
                       <div className="text-xs font-medium truncate text-(--color-text)">{link}</div>
                       <div className="text-[10px] text-(--color-text-muted)">{format(new Date(m.created_at || Date.now()), "dd/MM/yyyy HH:mm")}</div>
                    </div>
                 </a>
              ));
           })}
  ...
  return (
    <div className="flex flex-col gap-2 p-2">
      {items.map(m => (
        <a key={m.id} href={m.media_url || "#"} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 p-2 rounded-lg bg-(--color-surface-muted) border border-(--color-border) hover:bg-(--color-surface) transition-colors">
          <div className="w-8 h-8 rounded bg-(--color-primary)/10 flex items-center justify-center text-(--color-primary) shrink-0">
            <FiFileText size={14} />
          </div>
          <div className="flex-1 min-w-0">
            <div className="text-xs font-medium truncate text-(--color-text)">{m.body || "Documento"}</div>
            <div className="text-[10px] text-(--color-text-muted)">{format(new Date(m.created_at || Date.now()), "dd/MM/yyyy HH:mm")}</div>
          </div>
        </a>
      ))}
...
export function ContactInfoPanel({ chat, apiBase, onClose }: Props) {
  ...
  return (
    <div className="h-full flex flex-col bg-(--color-surface) border-l border-(--color-border) w-[420px] shadow-xl z-20 transition-all duration-300">
      <div className="p-4 border-b border-(--color-border) flex items-center justify-between bg-(--color-surface) backdrop-blur-sm">
        <h3 className="font-semibold text-sm text-(--color-heading)">Detalhes</h3>
        <button 
          onClick={onClose}
          className="p-1.5 hover:bg-(--color-surface-muted) rounded-full transition-colors text-(--color-text-muted) hover:text-(--color-text)"
        >
          <FiX size={16} />
        </button>
      </div>
      ...
      {/* Tabs */}
      <div className="flex items-center border-b border-(--color-border) px-2">
        {[
          ...
          >
            ...
            {tab.label}
            {activeTab === tab.id && (
              <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-(--color-primary) rounded-t-full" />
            )}
          </button>
        ))}
      </div>
        ...
        {activeTab === "info" && (
          <div className="p-4 space-y-6">
            ...
            <div className="flex flex-col items-center text-center">
              <div className="w-24 h-24 rounded-full bg-(--color-surface-muted) mb-3 overflow-hidden border-4 border-(--color-surface) shadow-lg ring-1 ring-(--color-border)">
                {contactAvatar ? (
                  <img src={contactAvatar} alt={contactName} className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full flex items-center justify-center bg-(--color-primary)/10 text-(--color-primary)">
                    <FiUser size={40} />
                  </div>
                )}
              </div>
              <h2 className="font-bold text-lg text-(--color-heading)">{contactName}</h2>
              <p className="text-sm text-(--color-text-muted) font-mono">{contactPhone}</p>
            </div>

            {/* Info List */}
            <div className="space-y-4">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-(--color-text-muted) uppercase tracking-wider">Contato</label>
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-(--color-text-muted) uppercase tracking-wider">Dados Adicionais</label>
                <div className="flex flex-col gap-2">
                   <EditableField 
                    label="Endereço" 
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-(--color-text-muted) uppercase tracking-wider">Sistema</label>
                <div className="grid grid-cols-2 gap-2">
                  <div className="p-2 rounded-lg bg-(--color-surface-muted) border border-(--color-border)">
                    <div className="text-[10px] text-(--color-text-muted) mb-1">Origem</div>
                  ...
                  <div className="p-2 rounded-lg bg-(--color-surface-muted) border border-(--color-border)">
                    <div className="text-[10px] text-(--color-text-muted) mb-1">Criado em</div>
            ...
            <div className="space-y-2 pt-2 border-t border-(--color-border)">
              ...
              <div className="flex flex-wrap gap-2">
                {localChat.tag_ids && localChat.tag_ids.length > 0 ? (
                   <div className="text-xs text-(--color-text-muted) italic">Tags gerenciadas no cabeçalho</div>
                ) : (
                  <span className="text-xs text-(--color-text-muted) italic">Sem tags</span>
                )}
              </div>
            </div>
