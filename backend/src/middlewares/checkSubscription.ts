...
/**
 * Middleware para verificar se a empresa tem acesso a uma feature espec�fica
 */
 ...
 * Se o limite for atingido, retorna 403 com informa��es do limite
 */
...
/**
 * Middleware "soft limit" - apenas avisa se est� pr�ximo do limite, mas n�o bloqueia
 * Adiciona headers com informa��es de uso
 */
export function warnOnLimit(resource: keyof PlanLimits, warningThreshold = 0.8) {
  ...
  const limitCheck = await checkLimit(companyId, resource);
      ...
      // Adicionar headers com informa��es de uso
